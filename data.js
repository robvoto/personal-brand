  ...
  workSamples: {
    ...
    pages: {
      bpmnProcessMaps: {
        ...
        diagramSteps: [
          { title: 'Capture', body: 'Scraper captures the raw job record and keeps structured metadata separate from the description.' },
          { title: 'Normalise', body: 'Source fields are normalised so title, company, location, salary and work mode can be compared reliably.' },
          { title: 'Pre-check', body: 'Deterministic hard blockers and trust checks run before any expensive review.' },
          { title: 'Taxonomy', body: 'Title and O*NET occupation logic decide whether the role is near, far or uncertain.' },
          { title: 'Fit review', body: 'Reviewed jobs either use deterministic review or LLM requirement coverage.' },
          { title: 'Freeze', body: 'The score is band-clamped, hard blockers are applied, and the explanation is frozen for inspection.' },
          { title: 'Display', body: 'Freshness and viewed status adjust ranking at display time without changing the frozen score.' },
          { title: 'Feedback', body: 'Save, skip and apply actions feed learning without silently changing the rules.' },
        ],
        whyItMattered:
      ...
      businessRulesDecisionLogic: {
        ...
        diagramSteps: [
          ...
          { title: 'Taxonomy', body: 'Title and occupation logic decides near, far or uncertain before review continues.' },
          { title: 'Coverage', body: 'Requirement coverage is evidence for the grade and must name real capabilities.' },
          { title: 'Band', body: 'Grade clamps the score into the correct range instead of pretending to be a percentage.' },
          { title: 'Penalty', body: 'Hard blockers are applied after the clamp and can force the score near zero.' },
          { title: 'Freeze', body: 'The display score stays separate from the frozen score so ranking remains inspectable.' },
        ],
        whyItMattered:
      ...
      aiProductDecisionFlows: {
        ...
        diagramSteps: [
          ...
          { title: 'Research', body: 'Complex tasks can be gated for research approval before execution continues.' },
          { title: 'Risk review', body: 'The orchestrator returns LOW, MEDIUM, HIGH or UNKNOWN and logs a reason.' },
          { title: 'Approval', body: 'Unsafe work pauses for human approval; safe work proceeds automatically.' },
          { title: 'Plan', body: 'The agent asks for a compact implementation plan and reviews it.' },
          { title: 'Instruction', body: 'The instruction builder produces a bounded task brief for the coding agent.' },
          { title: 'Run', body: 'The controlled coding-agent runner executes the work with explicit boundaries.' },
